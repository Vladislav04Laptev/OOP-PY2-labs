...
class Library:
    ...
    def get_next_book_id(self) -> int:
        ...
        """
        help_list = [] #вспомогательный пустой список
        if not self.books: #если список пуст, то он интерпретируется как False, воспользуемся этим
            return 1
        else:
            #перебираем все эл-ты Book из переданного списка и заносим каждый id в новый список
            for b in self.books:
                help_list.append(b.id)
            help_list = set(help_list) #сортируем по возрастанию список из id книг

            #перебираем натуральные числа до того момента пока не попадётся число, которого нету в списке, его и возвращаем
            i = 1
            while True:
                if i not in help_list:
                    return i
                i += 1
    def get_index_by_book_id(self, id_search: int) -> int:
        ...
        for b in self.books: #перебор всех элементов списков
            if b.id == id_search: #проверка совпадения id элемента класса Book с искомым
